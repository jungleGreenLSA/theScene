...
> Sign-in methods on The Scene: **Google OAuth** (Step 5) and **email + password** (this step). That is the whole list.
...
## Step 6 -- Other providers (not used)

The Scene signs members in with **Google** or **email/password** only. Leave Facebook, Discord, Phone/SMS and every other provider **off** in Supabase — the app has no UI for them, and fewer enabled providers is a smaller surface to secure.
