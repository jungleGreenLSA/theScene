    ...
    <div style={{ position: 'relative' }}>
      ...
      {mentionRange && (matches.length > 0 || searching || mentionRange.q.length > 0) && (
        <div style={{
          ...
          background: 'var(--color-surface)', border: '1px solid rgba(242,169,0,0.35)', borderRadius: '8px',
          boxShadow: '0 8px 24px rgba(0,0,0,0.5)', overflow: 'hidden',
        }}>
          {searching && matches.length === 0 && (
            <div style={{ padding: '10px 12px', fontSize: '12px', color: 'var(--color-muted-light)' }}>Searching…</div>
          )}
          {!searching && matches.length === 0 && mentionRange.q.length > 0 && (
            <div style={{ padding: '10px 12px', fontSize: '12px', color: 'var(--color-muted-light)' }}>No users matching &ldquo;@{mentionRange.q}&rdquo;</div>
          )}
          {matches.map((m, i) => (
            <button
              key={m.id}
              ...
              style={{
                ...
                background: i === activeIdx ? 'rgba(242,169,0,0.12)' : 'transparent',
              }}
            >
              <div style={{ width: '28px', height: '28px', borderRadius: '50%', overflow: 'hidden', background: 'var(--color-surface-light)', backgroundImage: m.avatar_url ? `url(${m.avatar_url})` : 'none', backgroundSize: 'cover', backgroundPosition: 'center', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                {!m.avatar_url && <span style={{ fontSize: '11px', color: 'var(--color-muted)' }}>{m.username.charAt(0).toUpperCase()}</span>}
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <p style={{ fontSize: '13px', fontWeight: 600, color: 'var(--color-foreground)' }}>@{m.username}</p>
                {m.display_name && <p style={{ fontSize: '11px', color: 'var(--color-muted-light)' }}>{m.display_name}</p>}
              </div>
            </button>
          ))}
        </div>
