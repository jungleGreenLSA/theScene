  ...
  return (
    <form onSubmit={handleSubmit} className="glass p-6 mb-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
        <div>
          <label className="eyebrow mb-1.5 block">Photo</label>
          <input
            type="file"
            accept="image/jpeg,image/png,image/webp"
            onChange={(e) => setFile(e.target.files?.[0] || null)}
            className="input text-sm file:mr-4 file:py-1 file:px-3 file:rounded file:border-0 file:text-xs file:bg-teal-dim file:text-teal file:cursor-pointer"
            required
          />
        </div>
        {vehicles.length > 0 && (
          <div>
            <label className="eyebrow mb-1.5 block">Your Vehicle</label>
      ...
      <div className="mb-4">
        <label className="eyebrow mb-1.5 block">Caption (optional)</label>
        <input
          type="text"
          value={caption}
      ...
      <button type="submit" disabled={loading || !file} className="btn-primary text-xs disabled:opacity-50">
