...
export default function VehicleMods({ vehicleId }: { vehicleId: string }) {
  ...
  if (loading) return <p style={{ fontSize: '13px', color: 'var(--color-muted)' }}>Loading mods...</p>
                ...
                <div key={cat.key} style={{ padding: '14px', borderRadius: '8px', background: 'var(--color-surface-lowest)', border: '1px solid var(--color-border)' }}>
                  ...
                  {items.length > 0 && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                      {items.map(m => (
                        <div key={m.id} style={{ padding: '10px 12px', borderRadius: '6px', background: 'var(--color-surface-lowest)', border: '1px solid var(--color-border)' }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '8px' }}>
                            ...
                            <div style={{ display: 'flex', gap: '6px', flexShrink: 0 }}>
                              <button type="button" onClick={() => openEdit(m)} style={{ padding: '4px 10px', borderRadius: '4px', background: 'var(--color-surface-light)', border: '1px solid var(--color-border)', color: 'var(--color-muted-light)', fontSize: '11px', fontWeight: 600, cursor: 'pointer', minHeight: '32px' }}>Edit</button>
