    ...
    <>
      {noCover && (
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '10px', pointerEvents: 'none' }}>
          <label style={{ pointerEvents: 'auto', cursor: 'pointer', padding: '14px 28px', borderRadius: '10px', background: 'rgba(124,58,237,0.9)', border: '1px solid rgba(167,139,250,0.5)', color: 'white', fontSize: '14px', fontWeight: 700, display: 'inline-flex', alignItems: 'center', gap: '8px', boxShadow: '0 8px 24px rgba(0,0,0,0.4)', opacity: uploading === 'cover' ? 0.6 : 1 }}>
      ...
      {!noCover && (
        <div style={{ position: 'absolute', top: '12px', left: '12px', display: 'flex', gap: '8px', zIndex: 2 }}>
          <label style={{ cursor: 'pointer', padding: '8px 14px', borderRadius: '6px', background: 'rgba(12,12,20,0.92)', border: '1px solid rgba(167,139,250,0.4)', color: '#e2e4e9', fontSize: '12px', fontWeight: 600, display: 'inline-flex', alignItems: 'center', gap: '6px', backdropFilter: 'blur(8px)', opacity: uploading === 'cover' ? 0.6 : 1 }}>
            Change Banner
            <input type="file" accept="image/jpeg,image/png,image/webp" style={{ display: 'none' }} disabled={uploading !== null}
              onChange={(e) => { const f = e.target.files?.[0]; if (f) handleUpload(f, 'cover') }} />
          </label>
          <label style={{ cursor: 'pointer', padding: '8px 14px', borderRadius: '6px', background: 'rgba(12,12,20,0.92)', border: '1px solid rgba(167,139,250,0.4)', color: '#e2e4e9', fontSize: '12px', fontWeight: 600, display: 'inline-flex', alignItems: 'center', gap: '6px', backdropFilter: 'blur(8px)', opacity: uploading === 'logo' ? 0.6 : 1 }}>
      ...
      {!noCover && noLogo && (
        <div style={{ position: 'absolute', bottom: '12px', right: '12px', zIndex: 2 }}>
          <label style={{ cursor: 'pointer', padding: '8px 14px', borderRadius: '6px', background: 'rgba(124,58,237,0.9)', border: '1px solid rgba(167,139,250,0.5)', color: 'white', fontSize: '12px', fontWeight: 600, display: 'inline-flex', alignItems: 'center', gap: '6px', boxShadow: '0 4px 16px rgba(0,0,0,0.4)', opacity: uploading === 'logo' ? 0.6 : 1 }}>
