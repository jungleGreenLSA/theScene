  ...
  return (
    <div style={{ maxWidth: '700px', margin: '0 auto', padding: '24px', minHeight: '100vh' }}>
      ...
      <div className="panel" style={{ overflow: 'hidden', marginBottom: '20px' }}>
        <div style={{ height: '300px', background: 'var(--color-surface-light)', position: 'relative' }}>
          ...
          <span className="chip chip-purple" style={{ position: 'absolute', top: '12px', right: '12px', background: 'rgba(15,16,18,0.85)', textTransform: 'capitalize' }}>
            {vehicle.build_status?.replace(/_/g, ' ')}
          </span>
        </div>

        <div style={{ padding: '24px' }}>
          <h1 style={{ fontSize: 'clamp(22px, 5vw, 28px)', fontWeight: 700, color: 'var(--color-foreground)', marginBottom: '4px' }}>
            <span className="spec" style={{ fontSize: '0.65em', color: 'var(--color-muted-light)', letterSpacing: '0.05em', display: 'block', marginBottom: '2px' }}>{vehicle.year}</span>
            {vehicle.make} {vehicle.model}
          </h1>
          <p className="spec" style={{ fontSize: '16px', color: 'var(--color-accent)', marginTop: '4px' }}>{vehicle.color}</p>

          <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginTop: '16px', flexWrap: 'wrap' }}>
            <span style={{ fontSize: '12px', color: 'var(--color-muted)' }}>
              <span className="spec" style={{ color: 'var(--color-foreground)' }}>{vehicle.props_count || 0}</span>
              <span style={{ marginLeft: '4px' }}>props</span>
            </span>
            <span style={{ fontSize: '12px', color: 'var(--color-muted)' }}>
              <span className="spec" style={{ color: 'var(--color-foreground)' }}>{vehicle.view_count || 0}</span>
          ...
          {owner && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginTop: '16px', paddingTop: '16px', borderTop: '1px solid var(--color-border)' }}>
              <div style={{ width: '44px', height: '44px', borderRadius: '50%', background: 'var(--color-surface-light)', backgroundImage: owner.avatar_url ? `url(${owner.avatar_url})` : 'none', backgroundSize: 'cover', backgroundPosition: 'center', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, border: '1px solid var(--color-border)' }}>
                {!owner.avatar_url && <span style={{ fontSize: '16px', color: 'var(--color-accent)', fontWeight: 700 }}>{owner.username?.charAt(0).toUpperCase()}</span>}
              </div>
              <div>
                <p style={{ fontSize: '14px', fontWeight: 600, color: 'var(--color-foreground)' }}>{owner.display_name || owner.username}</p>
                <p style={{ fontSize: '12px', color: 'var(--color-muted-light)' }}>
      ...
      {specItems.length > 0 && (
        <div className="panel" style={{ padding: '24px', marginBottom: '20px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(min(100%, 140px), 1fr))', gap: '8px' }}>
            {specItems.map(s => (
              <div key={s.label} style={{ padding: '12px', background: 'var(--color-surface-lowest)', borderRadius: '8px', border: '1px solid var(--color-border)' }}>
                <p className="eyebrow" style={{ marginBottom: '4px', fontSize: '9px' }}>{s.label}</p>
                <p className="spec" style={{ color: 'var(--color-foreground)', fontSize: '13px' }}>{s.value}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {images && images.length > 0 && (
        <div style={{ marginBottom: '20px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(min(100%, 200px), 1fr))', gap: '8px' }}>
            {images.map((img, i) => (
              <div key={i} style={{ borderRadius: '8px', overflow: 'hidden', background: 'var(--color-surface-light)', aspectRatio: '3 / 2' }}>
        ...
        <div className="panel" style={{ padding: '24px', marginBottom: '20px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            {Object.entries(modsByCategory).map(([category, catMods]) => (
              <div key={category}>
                <p style={{ fontSize: '10px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '1px', color: 'var(--color-accent)', fontFamily: 'var(--font-mono, monospace)', marginBottom: '6px' }}>{category}</p>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                  {catMods!.map((mod, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: '8px', fontSize: '13px', color: 'var(--color-muted-light)', paddingLeft: '8px' }}>
                      <span style={{ color: 'rgba(242,169,0,0.5)', marginTop: '3px', flexShrink: 0 }}>—</span>
                      <span>
                        {mod.brand && <span className="spec" style={{ color: 'var(--color-foreground)', marginRight: '4px' }}>{mod.brand}</span>}
                        {mod.item}
                      </span>
                    </div>
                  ))}
      ...
      {/* CTA */}
      <div className="panel glow-teal" style={{ padding: '32px', textAlign: 'center', border: '1px solid rgba(242,169,0,0.2)' }}>
        <Image src="/images/logo.png" alt="The Scene" width={48} height={48} style={{ margin: '0 auto 16px', borderRadius: '50%' }} />
        <h2 style={{ fontSize: 'clamp(17px, 4vw, 20px)', fontWeight: 700, color: 'var(--color-foreground)', marginBottom: '8px' }}>
          Want to show off <span className="gradient-text">your</span> build?
        </h2>
        <p style={{ fontSize: '14px', color: 'var(--color-muted-light)', marginBottom: '24px', lineHeight: 1.6 }}>
          The Scene is the car community, reimagined. Build your garage, connect with enthusiasts, and discover events near you.
        </p>
        <Link href="/auth/register" className="btn-primary" style={{ display: 'inline-flex', padding: '14px 32px', fontSize: '14px' }}>
          Join The Scene &mdash; It&apos;s Free
        </Link>
      </div>

      {/* Footer */}
      <div style={{ textAlign: 'center', marginTop: '24px', paddingTop: '16px', borderTop: '1px solid var(--color-border)' }}>
        <p style={{ fontSize: '11px', color: 'var(--color-muted)' }}>Powered by <span style={{ color: 'var(--color-accent)' }}>The Scene</span> · thescene.fyi</p>
      </div>
    </div>
  )
}
