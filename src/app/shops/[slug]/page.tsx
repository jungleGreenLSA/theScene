  ...
  return (
    <div style={{ maxWidth: '900px', margin: '0 auto', padding: '80px 16px 40px' }}>
      ...
      <div className="glass glow-teal" style={{ overflow: 'hidden', marginBottom: '24px' }}>
        <div style={{ height: '220px', background: 'rgba(26,26,46,0.5)', position: 'relative', overflow: 'hidden' }}>
          {shop.cover_image_url ? (
            ...
            <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg, rgba(45,212,191,0.1), rgba(139,92,246,0.08))' }} />
          )}
        </div>
        <div style={{ padding: '24px' }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: '16px', flexWrap: 'wrap' }}>
            {shop.logo_url && (
              <div style={{ width: '72px', height: '72px', borderRadius: '50%', overflow: 'hidden', background: '#0c0c14', border: '2px solid rgba(45,212,191,0.3)', marginTop: '-48px', position: 'relative', zIndex: 1, flexShrink: 0 }}>
                <img src={shop.logo_url} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
              </div>
            )}
            <div style={{ flex: 1, minWidth: 0 }}>
              <h1 style={{ fontSize: '26px', fontWeight: 700, color: '#e4e1ed' }}>{shop.name}</h1>
              {shop.city && shop.state && (
                <p style={{ fontSize: '14px', color: '#9ca3af', marginTop: '4px' }}>{[shop.address, shop.city, shop.state, shop.zip_code].filter(Boolean).join(', ')}</p>
              )}
            </div>
          </div>
          ...
          {shop.specialties && shop.specialties.length > 0 && (
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginTop: '14px' }}>
              {shop.specialties.map((s: string) => (
                <span key={s} style={{ fontSize: '10px', textTransform: 'uppercase', letterSpacing: '1px', fontWeight: 600, padding: '4px 10px', borderRadius: '4px', background: 'rgba(45,212,191,0.08)', color: '#2dd4bf', border: '1px solid rgba(45,212,191,0.2)', fontFamily: 'var(--font-mono)' }}>{s}</span>
              ))}
            </div>
          )}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginTop: '16px', paddingTop: '14px', borderTop: '1px solid rgba(255,255,255,0.06)', flexWrap: 'wrap' }}>
            {shop.website && <a href={shop.website} target="_blank" rel="noopener" style={{ fontSize: '13px', color: '#2dd4bf' }}>Website</a>}
            {shop.instagram_handle && <a href={`https://instagram.com/${shop.instagram_handle}`} target="_blank" rel="noopener" style={{ fontSize: '13px', color: '#2dd4bf' }}>@{shop.instagram_handle}</a>}
            {shop.phone && <a href={`tel:${shop.phone}`} style={{ fontSize: '13px', color: '#2dd4bf' }}>{shop.phone}</a>}
          </div>
        </div>
      </div>
      ...
      <div className="glass" style={{ padding: '24px' }}>
        ...
        <h2 style={{ fontSize: '16px', fontWeight: 700, color: '#e4e1ed', marginBottom: '14px' }}>Builds tagged here <span className="spec">({tags?.length || 0})</span></h2>
        {!tags || tags.length === 0 ? (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(min(100%, 240px), 1fr))', gap: '12px' }}>
            {tags.map((t: any) => {
              ...
              return (
                <Link key={t.id} href={`/user/${v.owner.username}/${v.slug}`} className="card-hover group" style={{ display: 'block', padding: '12px', borderRadius: '8px', background: 'rgba(18,18,30,0.5)', border: '1px solid rgba(255,255,255,0.06)' }}>
                  ...
                  <p className="spec group-hover:text-teal transition-colors" style={{ fontSize: '13px', fontWeight: 600, color: '#e2e4e9' }}>{v.year} {v.make} {v.model}</p>
